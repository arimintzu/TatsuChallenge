...
namespace EZ_Pooling
{
    ...
    public class EZ_PoolManager : MonoBehaviour
    {
        ...
        /// </summary>
        private static void CreateMissingPrefabPool(Transform missingTrans, string name, Transform parent)
        {
            var newPrefabPool = new EZ_PrefabPool();

            //Set the new pool options here
            newPrefabPool.parentTransform = parent;
        ...
        {
            return Spawn(transToSpawn, position, rotation, parentTransform);
        }

        public static Transform Spawn(Transform transToSpawn, Vector3 position, Quaternion rotation, Transform parent)
        {
            ...
            if (!Pools.ContainsKey(name))
            {
                ...
                {
                    CreateMissingPrefabPool(transToSpawn, name, parent);
                }
                else
                {
            ...
            return Pools[name].Spawn(transToSpawn, position, rotation, parent);
        }
        /// <summary>
